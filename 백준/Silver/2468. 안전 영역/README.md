...
### 풀이
1. 처음 보드를 입력받을 때 최고 건물 높이를 찾아둔다. 문제에서 언급함 비 높이(1~100)를 모두 탐색하는 시간을 줄일 수 있음
2. search() 메소드: 델타 배열을 이용해서 구역을 검색한다. 방문한 적이 없고 비높이보다 건물이 높을 경우(잠기지 않은 경우)를 탐색하면서 연결된 안전 구역 덩어리를 카운팅한다. (dfs 탐색) 반복문을 돌면서 안전지역의 최댓값을 갱신해 나간다. 
3. 인덱스 에러를 막기 위해 배열의 크기를 하나씩 늘려서 설정했다. 
