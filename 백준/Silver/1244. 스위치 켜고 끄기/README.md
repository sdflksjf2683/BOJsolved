...
### 풀이
남학생의 스위치 조작은 어렵지 않았는데, 여학생의 스위치 조작 로직을 구현하면서 입력값과 배열의 인덱스를 맞춰주는 작업이 까다로워서 계속 실패가 떴다. 결국 처음부터 배열의 크기를 한 칸 늘려서 인덱스를 1부터 접근하는 방식으로 풀었다. 구현은 쉬워졌으나 메모리를 낭비하는 방식이라서 아쉬웠다.
