...
### 풀이
- 바이러스를 선택하는 경우의 수마다 시뮬레이션을 돌려보고 최소값을 찾는 문제. 기존의 map을 그대로 사용하면 안되고, 복사해서 사용한다는 점만 기억하면 크게 어렵진 않았다. 
- deepcopy(): 기존의 map을 복사하는 메소드
- comb(): 바이러스를 활성화시키는 조합을 생성하는 메소드. 바이러스 조합을 완성하면 bfs()를 호출하고 최솟값을 갱신해준다. 
+ 추가로 바이러스가 바이러스를 지나갈 수 있다는 점을 간과해서 조금 헤멨다. 역시나 조건을 꼼꼼히 읽고 풀이 계획을 철저하게 세우는 것을 목표로 해야 할 듯.
