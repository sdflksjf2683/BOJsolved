...
### 풀이
- 먹을 수 있는 물고기들을 찾아서 큐에 넣어두고 더 위쪽, 왼쪽에 있는 것을 먹도록 한다. 이 때 객체를 생성하고 Priority Queue에 넣어서 자동으로 조건에 맞는 물고기를 먹을 수 있도록 하려고 했는데, 먹을 물고기를 넣어두는 큐와 움직일 수 있는 칸을 넣어두는 큐를 한꺼번에 사용하려고 하다보니 자꾸 오류가 났다. 
- 문제를 꼼꼼하게 읽지 않아서 놓치는 부분이 생겼고 이를 너무 늦게 알아채서 시간 내에 코드를 제출하지 못했다. 실전에서는 종이에 구현해야 할 조건들을 잘 적어두고 꼼꼼하게 계획을 짠 뒤 코드를 짜는 습관을 들여야 할듯.
