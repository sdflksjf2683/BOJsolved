...
- 사용한 클래스
	- Land class
		...
		- 섬의 좌표를 변수로 갖는다.
	- Bridge class
		- 섬끼리 연결하는 다리 정보를 저장하는 클래스
		- 다리가 시작하는 섬, 끝나는 섬, 다리의 길이를 변수로 갖는다.
		- 다리의 길이를 기준으로 오름차순 정렬한다(Comparable<>)

1. 각 섬에 고유 번호 붙이기(정점 구분) - BFS
	- 섬 번호를 1부터 시작하기 위해 입력받을 때 -1을 곱해서 입력을 받아준다.
	- findLand() 메소드
		- bfs 탐색을 하면서 연결된 부분에 같은 번호를 붙여준다. 
2. 다리 놓기 - BFS, DFS
	- map을 돌면서 섬이면 다리를 놓고, 가능한 다리(길이 2이상, 섬과 섬 연결)이면 리스트에 저장한다.
	- makeBridge() 메소드
		- 해당 구역부터 bfs 탐색을 하면서 바다가 있는 방향을 찾는다. 
		- 바다를 발견하면 해당 방향으로 dfs 탐색을 진행한다. 탐색을 진행하다가 시작 섬과 다른 번호인 섬을 만났고 그 때 다리의 길이가 2 이상이면 가능한 다리이므로 리스트에 저장한다.
3. 최단 경로 찾기 - MST
	-  다리를 저장한 리스트를 오름차순 정렬한 후, kruscal 알고리즘을 사용하여 모든 섬을 잇는 다리의 최단 길이 합을 구한다. 이 때 연결한 섬의 개수를 카운트 해둔다. 만약 카운트한 수가 섬의 개수보다 적을 경우에는 모든 섬을 이을 수 없으므로 -1을 출력한다. 
